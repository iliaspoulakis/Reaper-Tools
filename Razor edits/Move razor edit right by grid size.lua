...
-- Move razor edit to the right by one grid unit
...
-- Move edit cursor to the right by one grid unit
if not has_razor_edit then
    local track = reaper.GetLastTouchedTrack()
    local cursor_pos = reaper.GetCursorPosition()
    local _, _, _, full_beats = reaper.TimeMap2_timeToBeats(0, cursor_pos)
    local _, denom = reaper.TimeMap_GetTimeSigAtTime(0, cursor_pos + 0.0001)
