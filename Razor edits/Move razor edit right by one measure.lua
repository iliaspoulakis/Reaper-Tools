...
-- Move razor edit to the right by one measure
...
for i = 0, reaper.CountTracks(0) - 1 do
    ...
    if razor_edit ~= '' then
        ...
        for edit in razor_edit:gmatch('.- .- ".-"%s*') do
            ...
            local start_nom = reaper.TimeMap_GetTimeSigAtTime(0, start_pos + 0.0001)
            local new_start_beat = start_beat + start_nom
            local new_end_beat = end_beat + start_nom
...
-- Move edit cursor one measure to the right
if not has_razor_edit then
...
reaper.PreventUIRefresh(-1)
reaper.Undo_EndBlock('Move razor edit right by one measure', -1)
